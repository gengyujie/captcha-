...
y0 = tf.compat.v1.placeholder(tf.float32, [None])
y1 = tf.compat.v1.placeholder(tf.float32, [None])
y2 = tf.compat.v1.placeholder(tf.float32, [None])
y3 = tf.compat.v1.placeholder(tf.float32, [None])
...
def read_and_decode(filename):
    ...
    features = tf.io.parse_single_example(serialized_example,
                                       features={
                                           ...
                                           'label0': tf.io.FixedLenFeature([], tf.int64),
...
with Sess as sess:
    ...
    # 优化total_loss
    optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=lr).minimize(total_loss)
